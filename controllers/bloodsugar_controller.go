...
// UPDATE BLOOD SUGAR METRIC
func UpdateBloodSugarMetric(c *gin.Context) {
	userID, exists := c.Get("user_id")
	if !exists {
		c.JSON(http.StatusUnauthorized, gin.H{"error": "unauthorized"})
		return
	...
	metric.DateTime = input.DateTime
	metric.BloodSugarData = input.BloodSugarData
	metric.Context = input.Context

	if err := database.DB.Save(&metric).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "failed to update blood sugar metric"})
		return
	}

	c.JSON(http.StatusOK, gin.H{
		"message": "blood sugar metric updated successfully",
	})
}
